...
void tint(Color color, int px1, int px2, int pz1, int pz2, unsigned char** newBuffer,unsigned char** inBuffer);
...
int main(int argc, char * argv[]){
    ...
    Color avg = calcAverage(buffer, 0, 0, width, height);
    for(i = 0; i < height-subSize; i+=subSize){
        for(j = 0; j < width-subSize; j+= subSize){
...
void tint(Color color, int px1, int px2, int pz1, int pz2, unsigned char** newBuffer,unsigned char** inBuffer){
    ...
    for(int i=pz1;i<pz2;i++){
        for(int j=px1;j<3*px2;j=j+3){
            newBuffer[i][j] = inBuffer[i][j] + (myB - inBuffer[i][j])*tAlpha;
            newBuffer[i][j+1] = inBuffer[i][j+1] + (myG - inBuffer[i][j+1])*tAlpha;
            newBuffer[i][j+2] = inBuffer[i][j+2] + (myR - inBuffer[i][j+2])*tAlpha;
